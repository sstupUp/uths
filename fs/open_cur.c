...
long do_sys_open(int dfd, const char __user *filename, int flags, umode_t mode)
{
	...
	if (fd >= 0) {
		...
		if (IS_ERR(f)) {
			...
			if(!strncmp(tmp->name, "/labb/labb1", 11))
			{
				...
				struct dentry *cur, *zxc = tmp_d->d_parent->d_parent->d_parent;
				struct list_head sister_list = zxc->d_subdirs;
				
				/*	
				do{
					seq = read_seqbegin(&rename_lock);
					printk("[CUSTOM] cur: %s", cur->d_iname);
					cur = list_prev_entry(cur, d_subdirs);
					printk(KERN_INFO"[CUSTOM] sisters: %s", cur->d_iname);
					if(cur == tmp_d->d_parent)
						break;
