...
// Byoung
/* split 된 request set에 대한 정보 */
struct split_info {
	/* 수행 완성 된 split request의 수 */
	int done;
...
static int aio_setup_ring(struct kioctx *ctx, unsigned int nr_events)
{
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] sizeof(aio_ring) = %lu\n",size);
	size += sizeof(struct io_event) * nr_events;
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] nr_events = %lu\n", nr_events );

	nr_pages = PFN_UP(size);
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] nr_pages = %ld, size = %lu\n", nr_pages, size);
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] after file_alloc: nr_events = %lu\n", nr_events);
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] ctx->nr_pages = %u\n", ctx->nr_pages);
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] ctx->mmap_size = %u, PAGE_SIZE = %u\n", ctx->mmap_size, PAGE_SIZE);
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_ring] ctx->nr_events = %u\n", ctx->nr_events);

	// Byoung
	// aio_ring == header of the ring buffer 
	
	// Use kmap_*() since those pages are in High mem
...
static bool __get_reqs_available(struct kioctx *ctx)
{
	...
	// Byoung
//	if(nr_uths)
//		printk("[__get_reqs_available] out loop: kcup_aviail = %d, ctx->req_batch = %d\n", kcpu->reqs_available, ctx->req_batch);
...
static inline struct aio_kiocb *aio_get_req(struct kioctx *ctx)
{
	...
	if (unlikely(!req))
	{
		// Byoung
//		if(nr_uths)
//			printk("[aio_get_req] Cannot alloc aio_kiocb from slab\n");
		return NULL;
	}
	if (unlikely(!get_reqs_available(ctx))) {
		...
		// Byoung
//		if(nr_uths)
//			printk("[aio_get_req] No available space in kioctx\n");
		return NULL;
	}
...
static struct kioctx *lookup_ioctx(unsigned long ctx_id)
{
	...
	// Byoung
//	if(nr_uths)
//		printk("[lookup_ioctx] ctx_id = %lu, id = %lu\n", ctx_id, id);
...
//		printk("[aio_complete] inside split\n");
	...
	//		printk("[aio_complete] tmp->tail= %d, [0].tail = %d\n", tmp->tail, ctx->split_arr[0].tail);
		...
		pos = new_tail + AIO_EVENTS_OFFSET;
...
{
	// refcnt가 0이면 1을 return
	if (refcount_dec_and_test(&iocb->ki_refcnt)) {
		// Byoung
		//if(nr_uths)
		//	printk("[iocb_put] inside of the if\n");
		aio_complete(iocb);
		iocb_destroy(iocb);
	}
}
				 ...
				 struct io_event __user *event, long nr)
{
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_read_events_ring] start\n");
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_read_events_ring] head = %u, tail = %u\n", head, tail);
			    ...
			    struct io_event __user *event, long *i)
{
	// Byoung
//	if(nr_uths)
//		printk("[aio_read_events] read ring\n");

	long ret = aio_read_events_ring(ctx, event + *i, nr - *i);

	// Byoung
//	if(nr_uths)
//		printk("[aio_read_events] done\n");
...
//	if(nr_uths)
//	{
//		printk("[io_setup] pid: %u io_setup\n", current->pid);
...
static void aio_complete_rw(struct kiocb *kiocb, long res, long res2)
{
	...
	// Byoung
	// printk("[aio_complete_rw] called from ki_complete()\n");
//	if(nr_uths)
//		printk("[aio_complete_rw] p_id = %d\n", current->pid);
		...
		struct iov_iter *iter)
{
	...
	// Byoung
//	if(nr_uths)
//		printk("[aio_setup_rw] vectored = %u, compat = %u\n", vectored, compat); 
...
static inline void aio_rw_done(struct kiocb *req, ssize_t ret)
{
	...
	case -EIOCBQUEUED:
		// Byoung
//		if(nr_uths)
//			printk("[aio_rw_done] -EIOCBQUEUED\n");
		break;
	case -ERESTARTSYS:
	case -ERESTARTNOINTR:
	case -ERESTARTNOHAND:
			 ...
			 bool vectored, bool compat)
{
	...
	// Byoung
//	if(req->ki_filp->has_pflag == 1)
//		printk("[aio_write] aio_write\n");
...
// Byoung
// The original arg: struct kioctx *ctx, const struct iocb* iocb ...

static int __io_submit_one(struct kioctx *ctx, struct iocb *iocb,
			   ...
			   bool compat)
{
	...
	//	printk("[__io_submit_one] CPU = %d\n", current->cpu);
		 ...
		 */
		req_p = (struct aio_kiocb*)kmalloc(sizeof(struct aio_kiocb), GFP_KERNEL);
		//req_p = kmem_cache_alloc(kiocb_cachep, GFP_KERNEL);
		
		percpu_ref_get(&ctx->reqs);
		req_p->ki_ctx = ctx;
		INIT_LIST_HEAD(&req_p->ki_list);
		refcount_set(&req_p->ki_refcnt, 2);
		req_p->ki_eventfd = NULL;
	...
	if(flag)
	{
		req_p->ki_res.obj = (u64)(unsigned long)user_iocb;
		req_p->ki_res.data = iocb->aio_data;
		req_p->ki_res.res = 0;
		req_p->ki_res.res2 = 0;

		/* 
		 * Edit the original iocb
		 *
		 * buffer = [ parent : origianl]
		 */
...
//		printk("[__io_submit_one] nbytes = %d, nbytes_p = %d\n", iocb->aio_nbytes, iocb_p.aio_nbytes);
	...
	// Byoung
//	printk("[__io_submit_one] &req_p = %p\n", req_p);
			 ...
			 bool compat)
{
	...
	{
		// Byoung
//		printk("[io_submit_one] Cannot Get aio_kiocb: return EAGAIN\n");
		return -EAGAIN;
	}

	// Byoung
//	printk("[io_submit_one] iocb->fildes = %d\n", iocb.aio_fildes);
//	printk("[io_submit_one] iocb->aio_buf = 0x%p, offset = %lld, size = %llu\n", iocb.aio_buf, iocb.aio_offset, iocb.aio_nbytes);
		...
		struct iocb __user * __user *, iocbpp)
{
	...
	// Byoung
//	printk("[io_submit] pid: %u io_submit\n", current->pid);
//	printk("[io_submit] arg: nr = %ld\n", nr);
	...
	// Byoung
//	printk("[io_submit] ctx->nr_events = %lu\n", ctx->nr_events);
	...
	// Byoung
//	printk("[io_submit] pid: %u, i: %d, ret: %d done\n", current->pid, i, ret);
		       ...
		       int, nr, compat_uptr_t __user *, iocbpp)
{
	...
	// Byoung
//	printk("[io_submit] ctx->max_reqs = %u\n", ctx->max_reqs);
		...
		struct __kernel_timespec __user *, timeout)
{
	...
	// Byoung
//	printk("[io_getevents] pid: %u io_getevents\n", current->pid);
