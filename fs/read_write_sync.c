...
ssize_t ksys_write(unsigned int fd, const char __user *buf, size_t count)
{
	...
	//	struct mount* tmp = real_mount(f.file->f_path.mnt);
	//	struct inode* tmp_i = file_inode(f.file);
	//	printk("[ksys_write] %s | has_pflag = %d, used_pflag = %d, inode = 0x%08x, security = 0x%08x", tmp->mnt_devname, f.file->has_pflag, f.file->used_pflag, tmp_i, f.file->f_security);
...
//		printk("[ksys_write] Just file");
		...
		// Byoung
		//if(f.file->has_pflag == -2)
		//{	//Profile(1, ret, vfs_write, f.file, buf, count, ppos);	
	...
	// return flag that is some how mixed-up
	// version of ret and ret_p
	return ret;
}
...
//	{
//		ssize_t ret;
		//Profile(1, ret, ksys_write, fd, buf, count);
//		return ret;
//	}
//	else
