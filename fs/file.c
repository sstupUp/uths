...
static struct fdtable * alloc_fdtable(unsigned int nr)
{
	...
	// /////
	void *data2;
	data2 = kvmalloc_array(nr, sizeof(struct file *), GFP_KERNEL_ACCOUNT);
	if(!data2)
		goto out_fdt;
	fdt->fd_p = data2;

	data2 = kvmalloc(max_t(size_t, 2 *nr / BITS_PER_BYTE + BITBIT_SIZE(nr), L1_CACHE_BYTES), GFP_KERNEL_ACCOUNT);
	if(!data2)
		goto out_arr;
...
struct files_struct *dup_fd(struct files_struct *oldf, int *errorp)
{
	...
	*errorp = -ENOMEM;
	...
	// Byoung	
	new_fdt->fd_p = &newf->fd_p_array[0];
	//
...
repeat:
	...
	if (rcu_access_pointer(fdt->fd[fd]) != NULL) {
		...
		// Byoung
		if(rcu_access_pointer(fdt->fd_p[fd]) != NULL)
		{	
		//	printk("alloc_fd_p: slot %d not NULL\n", fd);
			rcu_assign_pointer(fdt->fd_p[fd], NULL);
		}
	
		//printk("[__alloc_fd] after fd_p");
	}
...
//	printk("[__fd_install] before is_pflag check");
	...
	//	BUG_ON(fdt->fd_p[fd] != NULL);
		rcu_assign_pointer(fdt->fd_p[fd], file);
		rcu_read_unlock_sched();
		return;
	}
	//
...
//	printk("[__close_fd] fd = %2d, fd[fd] = 0x%08x, fd_p[fd] = 0x%08x", fd, fdt->fd[fd], fdt->fd_p[fd]);		
...
loop:
	...
	//printk("[__fget] current->files parent: %s", file->f_path.dentry->d_parent->d_name.name);
